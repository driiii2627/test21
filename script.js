...
// Simulating a database with new properties for sorting
const gamesData = [
    { 
        id: 1, 
        title: "Minecraft", 
        ...
        hasAddons: true, 
        addons: [
            { categoryKey: "texturas", icon: "paintbrush", files: [{ name: "Faithful 32x" }, { name: "Bare Bones" }] }, 
            { categoryKey: "shaders", icon: "sun", files: [{ name: "BSL Shaders" }] }
        ],
        useShorteners: false,
        defaultLink: "#",
        downloadLinks: {},
        hasInfo: true,
        info: { version: "1.21.0", size: "150 MB", requirements: "Android 5.0+" },
        releaseDate: "2025-08-10", // YYYY-MM-DD
        downloads: 1523 
    },
    { 
        id: 2, 
    ...
    // Add more games with releaseDate and downloads for better testing
    { 
        id: 3, 
        title: "Stardew Valley", 
        image: "https://placehold.co/400x300/f59e0b/ffffff?text=Stardew+Valley", 
        ...
        downloadLinks: {},
        hasInfo: true,
        info: { version: "1.5.6", size: "300 MB", requirements: "Android 4.4+" },
     ...
     { 
        ...
        image: "https://placehold.co/400x300/3b82f6/ffffff?text=Genshin+Impact", 
        ...
        downloadLinks: {},
        hasInfo: true,
        info: { version: "4.5", size: "20 GB", requirements: "Android 8.0+" },
...
const loadDownloadCounts = () => {
    const counts = JSON.parse(localStorage.getItem('gameDownloadCounts')) || {};
    gamesData.forEach(game => {
        if (counts[game.id]) {
            game.downloads = counts[game.id];
        }
    });
};

const incrementDownloadCount = (gameId) => {
    const game = gamesData.find(g => g.id === gameId);
    if (game) {
        game.downloads = (game.downloads || 0) + 1;
        const counts = JSON.parse(localStorage.getItem('gameDownloadCounts')) || {};
        counts[game.id] = game.downloads;
        localStorage.setItem('gameDownloadCounts', JSON.stringify(counts));
    }
};
...
const createGameCardHTML = (game) => {
    ...
    const downloadButtonHTML = game.useShorteners && Object.keys(game.downloadLinks).length > 0
        ...
        : `<a href="${game.defaultLink || '#'}" class="w-full flex-grow card-button text-center py-2 px-3 rounded-lg font-semibold flex items-center justify-center space-x-2 text-sm download-link" data-game-id="${game.id}">
              <i data-lucide="download" class="w-4 h-4"></i>
              <span data-translate-key="download_button">${translations[currentLang]?.download_button || 'Baixar'}</span>
          </a>`;
...
const renderGames = (gamesToRender, gridElement) => {
    if (gridElement) {
        gridElement.innerHTML = gamesToRender
...
const renderAllGames = () => {
    ...
    renderGames(gamesData.filter(g => g.featured), featuredGrid);
    renderGames(gamesData, allGamesGrid);
...
const updateFilters = () => {
    ...
    const searchTerm = (desktopSearchInput.value || mobileSearchInput.value).toLowerCase();
    
    // --- Filtering Logic ---
    let filteredGames = gamesData.filter(game => {
        const titleMatch = game.title.toLowerCase().includes(searchTerm);
        const categoryMatch = activeCategories.length === 0 || activeCategories.every(cat => game.categories.includes(cat));
    ...
    // 'default' sort doesn't need specific logic, it maintains the original order from gamesData.

    // --- Rendering Logic ---
    const allGamesGrid = $('#all-games-grid');
    renderGames(filteredGames, allGamesGrid);
    
    const isFiltering = searchTerm.length > 0 || activeCategories.length > 0 || currentSort !== 'default';

    $('#featured-section')?.classList.toggle('hidden', isFiltering);
    $('#all-games-title').textContent = isFiltering 
        ? translations[localStorage.getItem('language') || 'pt'].search_results_title
        : translations[localStorage.getItem('language') || 'pt'].all_games_title;

    $('#noResultsMessage')?.classList.toggle('hidden', filteredGames.length > 0);
...
const generateCategoryFilters = () => {
    ...
    categories.forEach(category => {
        const key = `genre_${category.toLowerCase().replace(/ /g, '_')}`;
        const translatedCategory = translations[currentLang]?.[key] || category;
        filterButtonsHTML += `<button class="category-filter flex-shrink-0 px-4 py-2 rounded-lg text-sm font-semibold" data-category="${category}">${translatedCategory}</button>`;
    });
    ...
    $$('.category-filter').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            // Toggle category in activeCategories array
...
const openDownloadModal = (gameId) => {
    ...
    incrementDownloadCount(gameId); // Increment count when modal is opened
...
const setupCategoryScroller = () => {
    ...
    // Use a timeout to ensure the content is fully loaded before checking scroll
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        const downloadModalBtn = e.target.closest('.open-download-modal-btn');
        if (downloadModalBtn) {
            openDownloadModal(parseInt(downloadModalBtn.dataset.gameId, 10));
        }

        const directDownloadLink = e.target.closest('a.download-link');
        if (directDownloadLink) {
            incrementDownloadCount(parseInt(directDownloadLink.dataset.gameId, 10));
        }
