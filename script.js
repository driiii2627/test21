...
const createApkCardHTML = (apk) => {
    return `
        <div class="glass-card rounded-2xl flex items-center p-4">
            <img src="${apk.image}" alt="${apk.title}" class="w-16 h-16 rounded-lg object-cover mr-4">
            <div class="flex-grow">
                <h3 class="font-bold text-lg text-primary">${apk.title}</h3>
                <p class="text-sm text-muted">Version ${apk.version} | Updated: ${apk.updated}</p>
            </div>
            <a href="#" class="card-button p-2 rounded-lg">
...
const showPage = (pageId) => {
    const pages = [$('#page-home'), $('#page-apks'), $('#page-404'), $('#page-announcements'), $('#page-terms'), $('#page-privacy')];
    pages.forEach(page => {
